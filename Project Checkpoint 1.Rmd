...
```{r}
minDist<-function(busLongs, busLats, bikeLong, bikeLat){
   busLongLat<-matrix(c(busLongs, busLats),ncol=2)
   curMin<-min(apply(busLongLat,FUN=distHaversine,MARGIN=1,p2=c(bikeLong, bikeLat)))
   return(curMin)
}

niceRideLocationsDist<-niceRideLocations%>%
   group_by(Terminal)%>%
   mutate(minDist = mapply(minDist, bikeLat=Latitude, bikeLong=Longitude, MoreArgs=list(busLongs=metroStopsLimited$site_longitude, busLats=metroStopsLimited$site_latitude)))
...
ggplot(niceRide2016, aes(x= DayOfWeek)) + geom_bar(position = "dodge",aes(fill=Account.type), alpha= 0.3)
```
...
minBikeLat<-min(niceRideLocations$Latitude)-ft1000
maxBikeLat<-max(niceRideLocations$Latitude)+ft1000

minBikeLong<-min(niceRideLocations$Longitude)-ft1000
maxBikeLong<-max(niceRideLocations$Longitude)+ft1000
...
busLatLongs<-matrix(c(metroStopsLimited$site_longitude, metroStopsLimited$site_latitude),ncol=2)
...
closeDF<-distanceDF%>%
   ...
   mutate(site_id=as.integer(metroStopsLimited$site_id))%>%
   filter(minDist<200)
...
group_by(gatheredSampledClose, niceRideTerminal)%>%
   summarise(n())

dim(gatheredSampledClose)
...
dim(niceRideSampledClose)

head(niceRideSampledClose)
```
